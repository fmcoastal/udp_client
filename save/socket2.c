...
main(int argc,char **argv) {
    ...
    if ( z < 0 ) {
        fprintf(stderr,
            "%s: write(%d,\"%s\",%d)\n",
            strerror(errno),s[1],cp,(int)strlen(cp));
    ...
    if ( z < 0 ) {
        fprintf(stderr,
            "%s: read(%d,buf,%d)\n",
            strerror(errno),s[0],(int)sizeof buf);
    ...
    if ( z < 0 ) {
        fprintf(stderr,
            "%s: write(%d,\"%s\",%d)\n",
            strerror(errno),s[0],cp,(int)strlen(cp));
    ...
    if ( z < 0 ) {
        fprintf(stderr,
            "%s: read(%d,buf,%d)\n",
            strerror(errno),s[1],(int)sizeof buf);
        return 3;   /* Failed read */
    }
...
The following code shows how to indicate that no further writes will be performed upon the local socket.  This code will cause any buffers to flush while leaving the Rx side open for an acknowledge. Shutdown will close ALL copies/duplicates of the handle caused by a fork or dup operation 

close still needs to be called to free the file/descriptor handle
*/
						
       z = shutdown(s[0],SHUT_WR);
       if ( z == -1 )
              perror("shutdown()");
